...
# Definir las reglas de producción para el análisis sintáctico (parser)
def p_statement(p):
...
def p_error(p):
    if p:
        print(f"Error de sintaxis en '{p.value}' en la posición {p.lexpos} (línea {p.lineno})")
    else:
...
def analyze_sql():
    ...
    if result['result'] is None:
        return jsonify({'error': 'Error de sintaxis en la consulta SQL', 'tokens': result['tokens']}), 400
